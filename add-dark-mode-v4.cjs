...
function processFile(filePath) {
  ...
  for (const [key, val] of Object.entries(replacements)) {
    // If key doesn't start with hover:, we must ensure it's not preceded by hover:
    let prefix = '(?<!dark\\:)';
    if (!key.startsWith('hover:')) {
      prefix = '(?<!dark\\:|hover\\:)';
    }
    const regex = new RegExp(`${prefix}\\b${key.replace(/:/g, '\\:')}\\b(?!\\s+dark\\:)`, 'g');
  ...
  for (const [key, val] of Object.entries(replacements)) {
    ...
    const regex = new RegExp(`${prefix}\\b${key.replace(/:/g, '\\:')}\\/(\\d+)\\b(?!\\s+dark\\:)`, 'g');
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('bg-stone-900') && lines[i].includes('text-white')) {
      lines[i] = lines[i].replace(/(?<!dark\:|hover\:)\btext-white\b(?!\s+dark\:)/g, 'text-white dark:text-stone-900');
    }
  }
  content = lines.join('\n');

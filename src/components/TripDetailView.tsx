...
import { ArrowLeft, CheckCircle2, Circle, Plus, Trash2, ChevronDown, ChevronUp, Box, Sparkles, PenLine, Download, ShieldCheck, Image as ImageIcon, X, Save, Edit3, Map as MapIcon, Car, Calendar, List as ListIcon } from 'lucide-react';
...
export default function TripDetailView({ trip, inventory, profile, customLists, updateTrip, onBack }: Props) {
  ...
  const [editTripStartDate, setEditTripStartDate] = useState(trip.startDate || '');
  const [editTripEndDate, setEditTripEndDate] = useState(trip.endDate || '');
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      updateTrip({ ...trip, imageUrl: base64 });
    } catch (err) {
  ...
  const handleSaveTrip = () => {
    ...
    updateTrip({
      ...
      duration: editTripStartDate && editTripEndDate ? `${editTripStartDate} ${t('trips.to')} ${editTripEndDate}` : trip.duration
    });
  ...
  const allSuggestions = [
    ...(SUGGESTED_ITEMS['All'] || []),
    ...(SUGGESTED_ITEMS[trip.tripType] || [])
  ];
  const uniqueSuggestions = Array.from(new Map(allSuggestions.map(item => [item.name, item])).values());
  const availableSuggestions = uniqueSuggestions.filter(
  ...
  return (
    <div className="space-y-6">
      <div className="relative rounded-3xl overflow-hidden mb-8 bg-stone-900">
        {trip.imageUrl ? (
          <img src={trip.imageUrl} alt={trip.name} className="w-full h-48 sm:h-64 object-cover opacity-60" />
        ) : (
        ...
        <div className="absolute inset-0 p-6 flex flex-col justify-between">
          <div className="flex items-center justify-between">
            <button 
              onClick={onBack}
              className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center hover:bg-white/30 transition-colors text-white"
            >
            ...
            <div className="flex items-center gap-2">
              ...
              {trip.imageUrl && (
                <button
                  onClick={() => updateTrip({ ...trip, imageUrl: undefined })}
                  className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center hover:bg-red-500/80 transition-colors text-white"
                  title={t('trips.removeBackground')}
                >
                  <X className="w-5 h-5" />
                </button>
              )}
              <button
                ...
                className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center hover:bg-white/30 transition-colors text-white"
                title={t('trips.changeBackground')}
              >
          ...
          <div className="flex items-end justify-between">
            <div className="text-white">
              ...
              <div className="flex items-center gap-3 mt-1 text-white/90 font-medium drop-shadow-md text-sm sm:text-base">
                <span>{t(`type.${trip.tripType}`, trip.tripType)}</span>
                {trip.transportationType && (
                  <>
                    <span>&bull;</span>
                    <span className="flex items-center gap-1"><Car className="w-4 h-4" /> {t(`transport.${trip.transportationType}`, trip.transportationType)}</span>
                  </>
                )}
                <span>&bull;</span>
                <span className="flex items-center gap-1"><Calendar className="w-4 h-4" /> {trip.startDate && trip.endDate ? `${trip.startDate} ${t('trips.to')} ${trip.endDate}` : trip.duration}</span>
              </div>
            </div>
            <button
              onClick={() => setIsAddModalOpen(true)}
              className="bg-white text-stone-900 hover:bg-stone-100 font-medium rounded-xl px-4 py-2.5 flex items-center gap-2 transition-colors shadow-lg"
            >
              <Plus className="w-5 h-5" />
              <span className="hidden sm:inline">{t('trips.addItems')}</span>
            </button>
          </div>
        </div>
      </div>
            ...
            <div key={category} className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
              ...
              {isExpanded && (
                <div className="border-t border-stone-100 px-2 py-2">
                  {items.map((item) => (
                    <div key={item.id} className="flex items-center justify-between hover:bg-stone-50 rounded-xl transition-colors group">
                      <button
                      ...
                      <button
                        ...
                        className="p-3 text-stone-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all focus:opacity-100"
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="relative bg-white rounded-3xl shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
            ...
            <div className="flex border-b border-stone-100 bg-stone-50/50 px-4 pt-2 overflow-x-auto hide-scrollbar">
              <button
                onClick={() => setAddTab('mustBring')}
                className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 whitespace-nowrap ${addTab === 'mustBring' ? 'border-amber-500 text-amber-700' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
              >
                <ShieldCheck className="w-4 h-4" /> {t('inventory.mustBring')}
              </button>
              <button
                onClick={() => setAddTab('gear')}
                className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 whitespace-nowrap ${addTab === 'gear' ? 'border-emerald-500 text-emerald-700' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
              >
                <Box className="w-4 h-4" /> {t('inventory.gear')}
              </button>
              <button
                onClick={() => setAddTab('suggested')}
                className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 whitespace-nowrap ${addTab === 'suggested' ? 'border-emerald-500 text-emerald-700' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
              >
                <Sparkles className="w-4 h-4" /> {t('trips.tabSuggested')}
              </button>
              <button
                onClick={() => setAddTab('lists')}
                className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 whitespace-nowrap ${addTab === 'lists' ? 'border-emerald-500 text-emerald-700' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
              >
                <ListIcon className="w-4 h-4" /> {t('inventory.lists')}
              </button>
              <button
                onClick={() => setAddTab('custom')}
                className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 whitespace-nowrap ${addTab === 'custom' ? 'border-emerald-500 text-emerald-700' : 'border-transparent text-stone-500 hover:text-stone-700'}`}
              >
            ...
            <div className="flex-1 overflow-y-auto p-6">
                ...
                <div className="space-y-3">
                  {availableSuggestions.length === 0 ? (
                    ...
                    availableSuggestions.map((item, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex items-center justify-between p-4 rounded-xl border border-stone-200 hover:border-emerald-500/30 hover:bg-emerald-50/30 transition-all">
                          <div>
                            <div className="flex items-center gap-2">
                              <p className="font-medium text-stone-900">{t(`item.${item.name}`, item.name)}</p>
                              {isMustBring && (
              ...
              {addTab === 'custom' && (
                <form onSubmit={handleAddCustom} className="space-y-4 max-w-md mx-auto py-4">
                  ...
                  <button
                    type="submit"
                    className="w-full bg-stone-900 hover:bg-stone-800 text-white font-medium rounded-xl px-6 py-3 flex items-center justify-center gap-2 transition-colors mt-6"
                  >
                    <Plus className="w-5 h-5" />
                    {t('trips.addToTrip')}
                  </button>
          ...
          <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            ...
            <div className="p-6 border-t border-stone-100 bg-stone-50 flex justify-end gap-3">
              ...
              <button
                ...
                className="bg-emerald-500 hover:bg-emerald-600 text-white font-medium rounded-xl px-6 py-2.5 flex items-center gap-2 transition-colors"
              >
